...
export default function MyItemsPage() {
  ...
  async function handleApprove(rental) {
    setBusy(rental.id);
    try {
      const { error } = await supabase.from("rental_transactions").update({ status: "active" }).eq("id", rental.id);
      ...
      await supabase.from("messages").insert({
        ...
        body: `✅ Your rental request for "${rental.items?.name}" (ID: ${rental.id}) has been APPROVED! Please pick it up on ${fmtDate(rental.start_date)}. Return by ${fmtDate(rental.expected_return_date)}.`,
      });

      // Schedule a reminder: if rental starts today or tomorrow, send pickup reminder
      const startDate = new Date(rental.start_date);
      const daysUntilStart = Math.ceil((startDate - new Date()) / 86400000);
      if (daysUntilStart <= 1) {
        await supabase.from("messages").insert({
          sender_id: user.id,
          recipient_id: rental.renter_id,
          body: `📅 Reminder: your pickup for "${rental.items?.name}" is ${daysUntilStart <= 0 ? "today" : "tomorrow"}!`,
        });
      }

      toast("Rental approved!", "success");
    } catch (err) {
      toast(err instanceof Error ? err.message : "Failed to approve rental.", "error");
    } finally {
      setBusy("");
      load();
    }
  ...
  async function handleDecline(rental) {
    setBusy(rental.id);
    try {
      const { error } = await supabase.from("rental_transactions").update({ status: "cancelled" }).eq("id", rental.id);
      if (error) throw error;
      await supabase.from("messages").insert({
        sender_id: user.id,
        recipient_id: rental.renter_id,
        body: `❌ Your rental request for "${rental.items?.name}" (ID: ${rental.id}) has been declined. Feel free to message the owner if you have questions.`,
      });
      toast("Rental declined.", "info");
    } catch (err) {
      toast(err instanceof Error ? err.message : "Failed to decline rental.", "error");
    } finally {
      setBusy("");
      load();
    }
